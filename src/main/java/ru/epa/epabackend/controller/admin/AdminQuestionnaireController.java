...
/**
 * Класс для работы с энпоинтами анкеты, доступ к которым имеет админ.
 * Админ может создавать анкеты, тогда статус анкет будет CREATED, изменять CREATED анкеты без изменения их id.
 * Сотрудники не видят анкеты админа со статусом CREATED.
 * Админ может изменять статус анкеты на SHARED,
 ...
 */
@Tag(name = "Admin: Анкеты", description = "API админа для работы с анкетами")
@Validated
@RestController
...
public class AdminQuestionnaireController {
    ...
    /**
     * Получение последней заполняемой анкеты админа
     ...
     */
    @Operation(summary = "Получение последней заполняемой анкеты админа",
